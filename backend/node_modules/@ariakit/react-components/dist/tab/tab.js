...
const useTab = createHook(function useTab({ store, getItem: getItemProp, ...props }) {
	...
	const panelId = useStoreState(store.panels, (state) => state.items.find((item) => item.tabId === id)?.id);
	...
	const hasActiveItem = useStoreState(store, (state) => !!store.item(state.activeId));
...
const Tab = memo(forwardRef(function Tab(props) {
	return createElement(TagName, useTab(props));
}));
//#endregion
export { Tab, useTab };
