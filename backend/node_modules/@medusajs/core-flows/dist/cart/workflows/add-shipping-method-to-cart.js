...
exports.addShippingMethodToCartWorkflow = (0, workflows_sdk_1.createWorkflow)(exports.addShippingMethodToCartWorkflowId, (input) => {
    ...
    const currentCollidingShippingProfileMethodIds = (0, workflows_sdk_1.transform)({ input, shippingOptions, cart }, ({ input, shippingOptions, cart }) => {
        const incomingProfileIds = new Set(input.options
            .map((o) => shippingOptions.find((so) => so.id === o.id)
            ?.shipping_profile_id)
            .filter(Boolean));
        return cart.shipping_methods
            .filter((sm) => !sm.shipping_option?.shipping_profile_id ||
