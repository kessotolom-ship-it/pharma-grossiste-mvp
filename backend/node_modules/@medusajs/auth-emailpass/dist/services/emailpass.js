...
class EmailPassAuthService extends utils_1.AbstractAuthModuleProvider {
    ...
    async upsertAuthIdentity(type, { email, password, actor_type, authIdentityService }) {
        const passwordHash = await this.hashPassword(password);
        const providerMetadata = type === "update"
            ? await this.getProviderMetadata_(email, authIdentityService)
            : {};
        providerMetadata.password = passwordHash;
        const authIdentity = type === "create"
    ...
    async getProviderMetadata_(entityId, authIdentityService) {
    ...
    sanitizeAuthIdentity_(authIdentity) {
    ...
    getProviderIdentity_(authIdentity) {
        return authIdentity.provider_identities?.find((pi) => pi.provider === this.provider);
    }
}
