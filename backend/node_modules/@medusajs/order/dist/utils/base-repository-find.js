...
function setFindMethods(klass, entity) {
    klass.prototype.find = async function find(options, context) {
        ...
        (0, utils_1.pruneFindOptionsAgainstMetadata)(manager.getDriver().getMetadata().get(entity.name), config.options);
        ...
        if (isRelatedEntity) {
            ...
            if (strategy === core_1.LoadStrategy.JOINED) {
                ...
                config.options.populate.push("order.shipping_methods.shipping_method.adjustments");
        ...
        if (config.options.populate.includes("shipping_methods.shipping_method.adjustments")) {
            // TODO: handle if populate is an object
            shouldLoadShippingAdjustments = true;
            config.options.populate.splice(config.options.populate.indexOf("shipping_methods.shipping_method.adjustments"), 1);
            config.options.populate.push("shipping_methods");
...
async function loadShippingAdjustments(manager, orders) {
    ...
    const shippingMethodsIdMap = new Map(shippingMethods.map((s) => [s.shipping_method.id, s.shipping_method]));
    ...
    const adjustments = await manager.find(_models_1.OrderShippingMethodAdjustment, {
        $or: params,
    });
    for (const adjustment of adjustments) {
        const shippingMethod = shippingMethodsIdMap.get(adjustment.shipping_method_id);
        if (shippingMethod) {
    ...
    if (isRelatedEntity) {
        ...
        if (hasRelation("shipping_methods")) {
            ...
            if (isSelectIn) {
                ...
                const fragment = (0, core_1.raw)((alias) => `"${alias}"."version" = (select "_sub0"."version" from "order" as "_sub0" where "_sub0"."id" = "${alias}"."order_id")`);
                popWhere.shipping_methods[fragment.toString()] = [];
            }
            else {
        ...
        if (hasRelation("items") || hasRelation("order.items")) {
            ...
            if (isSelectIn) {
                ...
                const fragment = (0, core_1.raw)((alias) => `"${alias}"."version" = (select "_sub0"."version" from "order" as "_sub0" where "_sub0"."id" = "${alias}"."order_id")`);
                popWhereOrder.items[fragment.toString()] = [];
            }
            else {
        ...
        if (hasRelation("shipping_methods")) {
            ...
            if (isSelectIn) {
                const fragment = (0, core_1.raw)((alias) => `"${alias}"."version" = (select "_sub0"."version" from "order" as "_sub0" where "_sub0"."id" = "${alias}"."order_id")`);
                popWhereOrder.shipping_methods[fragment.toString()] = [];
            }
            else {
