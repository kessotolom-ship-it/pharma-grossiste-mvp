...
class OrderModuleService extends utils_1.ModulesSdkUtils.MedusaService(generateMethodForModels) {
    ...
    async createOrders_(data, sharedContext = {}) {
        ...
        for (const data_ of data) {
            ...
            if (ord.currency_code) {
                ord.currency_code = (0, utils_1.normalizeCurrencyCode)(ord.currency_code);
            }
    ...
    async previewOrderChange(orderId, sharedContext = {}) {
        ...
        const { itemsToUpsert, shippingMethodsToUpsert, calculatedOrders, lineItemAdjustmentsToCreate, shippingMethodAdjustmentsToCreate, } = await (0, utils_2.applyChangesToOrder)([order], { [order.id]: sortedActions }, { addActionReferenceToObject: true });
        ...
        shippingMethodAdjustmentsToCreate, sharedContext);
    ...
    async includeTaxLinesAndAdjustmentsToPreview(order, itemsToUpsert, shippingMethodsToUpsert, lineItemAdjustmentsToCreate, shippingMethodAdjustmentsToCreate, sharedContext = {}) {
        const addedItems = {};
        ...
        if (Object.keys(addedShippingMethods).length > 0) {
            ...
            order.shipping_methods.forEach((sm, idx) => {
                ...
                const adjustments = shippingMethodAdjustmentsToCreate.filter((d) => d.shipping_method_id === sm.id);
    ...
    async revertLastChange_(order, sharedContext = {}) {
        ...
        const orderItems = await this.orderItemService_.list({
            order_id: order.id,
            version: currentVersion,
        }, { select: ["id", "version", "item_id"] }, sharedContext);
        const orderItemIds = orderItems.map((summary) => summary.id);
        updatePromises.push(this.orderItemService_.softDelete(orderItemIds, sharedContext));
        // Order Shipping
        const orderShippings = await this.orderShippingService_.list({
            order_id: order.id,
            version: currentVersion,
        }, { select: ["id", "version", "shipping_method_id"] }, sharedContext);
        const orderShippingIds = orderShippings.map((sh) => sh.id);
        updatePromises.push(this.orderShippingService_.softDelete(orderShippingIds, sharedContext));
        ...
        if (itemIds.length) {
            ...
            if (lineItemAdjustmentIds.length) {
                updatePromises.push(this.orderLineItemAdjustmentService_.softDelete(lineItemAdjustmentIds, sharedContext));
            }
        }
        const shippingMethodIds = orderShippings.map((orderShipping) => orderShipping.shipping_method_id);
        if (shippingMethodIds.length) {
            const shippingMethodAdjustments = await this.orderShippingMethodAdjustmentService_.list({
                shipping_method_id: shippingMethodIds,
                version: currentVersion,
            }, { select: ["id"] }, sharedContext);
            const shippingMethodAdjustmentIds = shippingMethodAdjustments.map((adj) => adj.id);
            if (shippingMethodAdjustmentIds.length) {
                updatePromises.push(this.orderShippingMethodAdjustmentService_.softDelete(shippingMethodAdjustmentIds, sharedContext));
            }
        }
        // Order Credit Lines
        const orderCreditLines = await this.orderCreditLineService_.list({
    ...
    async applyOrderChanges_(changeActions, sharedContext = {}) {
        ...
        const { itemsToUpsert, shippingMethodsToUpsert, summariesToUpsert, orderToUpdate, creditLinesToUpsert, lineItemAdjustmentsToCreate, shippingMethodAdjustmentsToCreate, } = await (0, utils_2.applyChangesToOrder)(orders, actionsMap, {
        ...
        const [_orderUpdate, _orderChangeActionUpdate, orderItems, _orderSummaryUpdate, orderShippingMethods, orderCreditLines,] = await (0, utils_1.promiseAll)([
            ...
            shippingMethodAdjustmentsToCreate.length
                ? this.orderShippingMethodAdjustmentService_.create(shippingMethodAdjustmentsToCreate, sharedContext)
                : null,
        ]);
        return {
            items: orderItems ?? [],
    ...
    async addOrderTransactions_(transactionData, sharedContext = {}) {
        ...
        const normalizedData = data.map((d) => ({
            ...d,
            currency_code: (0, utils_1.normalizeCurrencyCode)(d.currency_code ?? ""),
        }));
        const created = (await this.orderTransactionService_.create(normalizedData, sharedContext));
