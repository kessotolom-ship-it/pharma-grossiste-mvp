...
class PaymentModuleService extends utils_1.ModulesSdkUtils.MedusaService(generateMethodForModels) {
    ...
    async createPaymentCollections_(data, sharedContext) {
        const normalizedData = data.map((d) => ({
            ...
            currency_code: d.currency_code
                ? (0, utils_1.normalizeCurrencyCode)(d.currency_code)
                : d.currency_code,
        }));
        return await this.paymentCollectionService_.create(normalizedData, sharedContext);
    }
    // @ts-expect-error
    ...
    async upsertPaymentCollections_(data, sharedContext) {
        ...
        const forUpdate = input
            ...
            ...(element.currency_code ?
                { currency_code: (0, utils_1.normalizeCurrencyCode)(element.currency_code) } :
                {})
        }));
    ...
    async createPaymentSession(paymentCollectionId, input, sharedContext) {
        ...
        try {
            ...
            providerPaymentSession = await this.paymentProviderService_.createSession(input.provider_id, {
                ...
                amount: input.amount,
                currency_code: (0, utils_1.normalizeCurrencyCode)(input.currency_code),
            });
    ...
    async authorizePaymentSession_(session, data, status, sharedContext) {
        ...
        const payment = await this.paymentService_.create({
            ...
            currency_code: (0, utils_1.normalizeCurrencyCode)(session.currency_code),
    ...
    async refundPayment_(payment, data, sharedContext = {}) {
        ...
        const upperCurCode = payment.currency_code?.toUpperCase();
        const currencyEpsilon = (0, utils_1.getEpsilonFromDecimalPrecision)(utils_1.defaultCurrencies[upperCurCode]?.decimal_digits);
        if (utils_1.MathBN.lt(utils_1.MathBN.sub(capturedAmount, totalRefundedAmount), -currencyEpsilon)) {
    ...
    async cancelPayment(paymentId, sharedContext) {
        ...
        const { data } = await this.paymentProviderService_.cancelPayment(payment.provider_id, {
            data: payment.data,
            context: {
                idempotency_key: payment.id,
            },
        });
        await this.paymentService_.update({ id: paymentId, canceled_at: new Date(), data }, sharedContext);
    ...
    async deletePaymentMethods(data, sharedContext) {
        const input = Array.isArray(data) ? data : [data];
        await (0, utils_1.promiseAll)(input.map((item) => this.paymentProviderService_.deletePaymentMethod(item.provider_id, {
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], PaymentModuleService.prototype, "deletePaymentMethods", null);
__decorate([
    (0, utils_1.InjectManager)(),
    __param(1, (0, utils_1.MedusaContext)()),
    __metadata("design:type", Function),
