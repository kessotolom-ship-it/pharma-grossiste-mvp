...
exports.hasEslintConfig = hasEslintConfig;
exports.lintProject = lintProject;
exports.runLintStep = runLintStep;
...
function hasEslintConfig(dir) {
    return ESLINT_CONFIG_FILES.some((file) => node_fs_1.default.existsSync(node_path_1.default.join(dir, file)));
}
/**
 ...
 */
async function lintProject(opts) {
    const eslintModule = loadEslintModule(opts.cwd);
    if (!eslintModule) {
        opts.logger.warn("Linting skipped: the `eslint` package is not installed in this project. " +
    ...
    const cacheLocation = node_path_1.default.join(opts.cwd, ".medusa/cache/.eslintcache");
    node_fs_1.default.mkdirSync(node_path_1.default.dirname(cacheLocation), { recursive: true });
 ...
 */
async function runLintStep(opts) {
    ...
    try {
        ...
        const message = `Linting failed to run: ${typedError.message}. ` +
    ...
    if (result.errorCount > 0) {
        ...
        if (failOnError) {
            ...
            opts.logger.error(`Lint failed with ${result.errorCount} error(s).${suffix}`);
            process.exit(1);
        }
        const warningNote = result.warningCount > 0 ? ` and ${result.warningCount} warning(s)` : "";
        opts.logger.warn(`Lint found ${result.errorCount} error(s)${warningNote}. ` +
