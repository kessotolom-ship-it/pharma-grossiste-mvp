...
class PricingModuleService extends BaseClass {
    ...
    async calculatePrices(pricingFilters, pricingContext = { context: {} }, sharedContext = {}) {
        ...
        pricesSetPricesMap.forEach((prices, key) => {
             ...
             */
            const nonSalePrices = prices.filter((p) => p.price_list_type !== utils_1.PriceListType.SALE);
            const nonSalePriceListPrice = nonSalePrices.find((p) => p.price_list_id);
            let calculatedPrice = defaultPrice;
            let originalPrice = nonSalePriceListPrice ?? defaultPrice;
            /**
    ...
    normalizePrices(data, existingPrices, priceListId) {
        ...
        data?.forEach((price) => {
            ...
            if ((0, utils_1.isDefined)(entry.currency_code)) {
                entry.currency_code = (0, utils_1.normalizeCurrencyCode)(entry.currency_code);
            }
    ...
    async updatePriceLists_(data, sharedContext = {}) {
        ...
        const normalizedData = this.normalizePriceListDate(data).map((priceList) => {
            ...
            if ((0, utils_1.isPresent)(priceList.metadata)) {
                const existing = existingByIdMap.get(priceList.id);
                entry.metadata = (0, utils_1.mergeMetadata)(existing?.metadata ?? {}, priceList.metadata);
            }
...
const hashPrice = (price) => {
    ...
    if ("currency_code" in price) {
        parts.push(`cc:${(0, utils_1.normalizeCurrencyCode)(price.currency_code ?? "")}`);
    }
...
const buildPreNormalizationPriceConstraintsFromData = (prices, priceListIds) => {
    ...
    for (const price of prices) {
        ...
        if (!!price.currency_code) {
            constraint.currency_code = (0, utils_1.normalizeCurrencyCode)(price.currency_code);
        }
